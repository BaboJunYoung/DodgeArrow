...
class SkillState:
    ...
    def add(self, amount: int):
        ...
        if self.meter >= SKILL_METER_MAX:
            ...
            self.ready = True
            try:
                with open("skill_state_2p.json", "w", encoding="utf-8") as f:
                    json.dump(
                        {"skill_ready": True, "who": "2P", "timestamp": pygame.time.get_ticks()},
                        f,
                        ensure_ascii=False
                    )
            except Exception:
...
class Arrow:
    ...
    def _build_surface(self, angle_deg):
        ...
        tail_x = cx - 6
        f1 = [(tail_x, cy),
              (tail_x - ARROW_FEATHER_LEN, cy - ARROW_FEATHER_W),
...
def draw_hud(surf, score, skill: SkillState, small_active, small_remain_ms):
    pygame.draw.rect(surf, HUD_BG, pygame.Rect(0, 0, WIDTH, HUD_H))
    pygame.draw.line(surf, FRAME_COLOR, (0, HUD_H), (WIDTH, HUD_H), 2)
...
def main():
    ...
    play_rect = pygame.Rect(0, HUD_H, WIDTH, HEIGHT - HUD_H)
    ...
    while running_global:
        ...
        while not dead:
            ...
            draw_hud(screen, score, skill, small_active, remain_ms)
