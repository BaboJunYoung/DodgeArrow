...
class SkillState:
    ...
    def add(self, amount: int):
        ...
        if self.meter >= SKILL_METER_MAX:
            ...
            self.ready = True
            try:
                with open("skill_state.json", "w", encoding="utf-8") as f:
                    json.dump(
                        {"skill_ready": True, "who": "1P", "timestamp": pygame.time.get_ticks()},
                        f,
                        ensure_ascii=False
                    )
            except Exception:
...
def draw_hud(surf, score, skill: SkillState, slow_active, slow_remain_ms):
...
def main():
    ...
    while running_global:
        ...
        while not dead:
            ...
            draw_hud(screen, score, skill, slow_active, remain_ms)
            pygame.display.flip()
